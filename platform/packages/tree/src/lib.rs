...
impl<T> Tree<T> {
    ...
    #[inline]
    pub fn direct_children(&self) -> impl Iterator<Item = Node<'_, T>> {
        direct_children(self, 0, self.parent_indexes.len())
    }

    #[inline]
    pub fn depth_first_values_iter(&self) -> impl Iterator<Item = &T> {
        Some(&self.root)
            .into_iter()
    ...
    #[inline]
    pub fn depth_first_nodes_iter(&self) -> impl Iterator<Item = Node<'_, T>> {
        Some(self.root())
            .into_iter()
...
impl<'r, T> Node<'r, T> {
    ...
    #[inline]
    pub fn direct_children(&self) -> impl Iterator<Item = Node<'r, T>> + 'r {
        direct_children(
            self.tree,
    ...
    #[inline]
    pub fn depth_first_values_iter(&self) -> impl Iterator<Item = &'r T> {
    ...
    #[inline]
    pub fn depth_first_nodes_iter(&self) -> impl Iterator<Item = Node<'r, T>> + 'r {
        let (maybe_root, start_index) = self.index.map_or_else(
            || (Some(self.tree.root()), 0),
            |index| (None, index.get() - 1),
        );
