...
open_error_report_fd() {
  if ! eval "exec ${error_report_fd:?}>\"/${output_artifacts_dir:?}\
/error-report.txt\""
  then
...
rerun_as_unprivileged() {
  ...
  if ! "clean_dir_contents" "/${output_artifacts_dir:?}"
  then
    "error" "Failed to clean up \"/${output_artifacts_dir:?}\" during \
preparations!"
  fi
